...
**The state machine is event-driven from the protocol** (MCP 2025-11-25). A server is `connected`; it transitions to `needs-auth` precisely on a `401` + `WWW-Authenticate`; to `authenticating` while the browser session is open; back to `connected` after a successful Bearer retry; to `expired` / `needs-auth` on refresh failure; and to `error` on a non-401 failure (an unreachable server, malformed metadata, a cancelled or denied consent). `ConnectionStatusChip` renders this five-state machine (`connected` / `needs-auth` / `authenticating` / `expired` / `error`, as green / yellow / red + label); nothing else needs to *announce* connection state.
...
enum ElicitationResult {
    case accept(GeneratedContent?)  // form mode: validated field values; URL mode: nil (consent-to-open only)
